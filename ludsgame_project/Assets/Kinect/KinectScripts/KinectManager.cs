...
//	public bool NearMode = false;
	...
	public void SetPlayerTracked(bool playerTracked) {
		Player1Calibrated = playerTracked;
	}
	...
	void Update()
	{
		if(KinectInitialized)
		{
							...
							gestureData.progress >= 0.5f)
						{
							if(GetGestureProgress(gestureData.userId, KinectGestures.Gestures.Click) < 0.3f)
							{
								if(HandCursor1 != null)
								{
									...
									if(HandCursor1.GetComponent<RawImage>() == null)
									{
								...
								if(ControlMouseCursor)
								{
									...
									MouseControl.MouseMove(vCursorPos, GesturesDebugText);
								}
							}
						}
			...
			if(Player2Calibrated)
			{
				...
				// Check for complete gestures
				foreach(KinectGestures.GestureData gestureData in player2Gestures.ToArray())
				{
					if(gestureData.complete)
					{
						if(gestureData.gesture == KinectGestures.Gestures.Click)
							...
							gestureData.progress >= 0.5f)
						{
							if(GetGestureProgress(gestureData.userId, KinectGestures.Gestures.Click) < 0.3f)
							{
								if(HandCursor2 != null)
								{
									...
									if(HandCursor2.GetComponent<RawImage>() == null)
									{
