...
namespace Share.KinectUtils {
	public class KinectGestures {
		...
		private static SkeletonRecorder skeletonRecorder;
		private static uint currentFrameNumber;
		...
		public struct GestureData {
			...
			public float[] gestureTimes;
			public string frameNumbers;
			public uint initlalFrame;
			public uint finalFrame;
		}
		
		#region Gesture related constants, variables and functions
		...
		private static void SetGestureJoint(ref GestureData gestureData, float timestamp, int joint, Vector3 jointPos, int stateCount, uint frameNumber) {
			gestureData.gestureTimes = new float[stateCount];
			gestureData.gestureTimes[gestureData.state] = timestamp;
			
			gestureData.frameNumbers = frameNumber.ToString() + SEPARATOR;
			
			gestureData.joint = joint;
			gestureData.jointPos = jointPos;
			gestureData.timestamp = timestamp;
			gestureData.state++;
			gestureData.initlalFrame = frameNumber;
		}
		...
		private static void CheckPoseComplete(ref GestureData gestureData, float timestamp, Vector3 jointPos, bool isInPose, float durationToComplete, uint frameNumber) {
			if(isInPose) {
				...
				if(timeLeft >= durationToComplete) {
					...
					gestureData.finalFrame = frameNumber;
					
					if(skeletonRecorder == null){
						//skeletonRecorder = SkeletonRecorder.Instance;
					}
					gestureData.frameNumbers += frameNumber.ToString();
				//	skeletonRecorder.AddGesture(ref gestureData);
				}
			} else {
		...
		private static void SetScreenPos(uint userId, ref GestureData gestureData, ref Vector3[] jointsPos, ref bool[] jointsTracked) {
			...
			if(calculateCoords) {
				...
				if(jointsTracked[hipCenterIndex] && jointsTracked[shoulderCenterIndex] &&  jointsTracked[leftShoulderIndex] && jointsTracked[rightShoulderIndex]) {
		...
		private void LockHand(bool locked) {
			isLocked = locked;
		}
		...
		public static void CheckForGesture(uint userId, ref GestureData gestureData, float timestamp, ref Vector3[] jointsPos, ref bool[] jointsTracked, uint frameNumber) {
			...
			case Gestures.RaiseRightHand:
				...
				case 0:  // gesture detection
					if(jointsTracked[rightHandIndex] && jointsTracked[rightShoulderIndex] && (jointsPos[rightHandIndex].y - jointsPos[rightShoulderIndex].y) > 0.1f) {
						SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex], 2, frameNumber);
					}
					break;
					
				case 1:  // gesture complete
					bool isInPose = jointsTracked[rightHandIndex] && jointsTracked[rightShoulderIndex] && (jointsPos[rightHandIndex].y - jointsPos[rightShoulderIndex].y) > 0.1f;
					
					Vector3 jointPos = jointsPos[gestureData.joint];
					CheckPoseComplete(ref gestureData, timestamp, jointPos, isInPose, KinectWrapper.Constants.PoseCompleteDuration, frameNumber);
					break;
				}
				break;
				...
				case 0:  // gesture detection
					if(jointsTracked[leftHandIndex] && jointsTracked[leftShoulderIndex] &&
					   (jointsPos[leftHandIndex].y - jointsPos[leftShoulderIndex].y) > 0.1f) {
						SetGestureJoint(ref gestureData, timestamp, leftHandIndex, jointsPos[leftHandIndex], 2, frameNumber);
					}
					break;
			...
			case Gestures.RaiseHand:
				...
				case 0:  // gesture detection
					if(jointsTracked[rightHandIndex] && jointsTracked[rightShoulderIndex] && (jointsPos[rightHandIndex].y - jointsPos[rightShoulderIndex].y) > 0.1f) {
						SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex], 2, frameNumber);
					} else if(jointsTracked[leftHandIndex] && jointsTracked[leftShoulderIndex] && (jointsPos[leftHandIndex].y - jointsPos[leftShoulderIndex].y) > 0.1f) {
						SetGestureJoint(ref gestureData, timestamp, leftHandIndex, jointsPos[leftHandIndex], 2, frameNumber);
					}
					break;
				...
				case 1:  // gesture complete
					bool isInPose = false;
					if(jointsTracked[rightHandIndex] && jointsTracked[rightShoulderIndex] && (jointsPos[rightHandIndex].y - jointsPos[rightShoulderIndex].y) > 0.1f ||
					   jointsTracked[leftHandIndex] && jointsTracked[leftShoulderIndex] && (jointsPos[leftHandIndex].y - jointsPos[leftShoulderIndex].y) > 0.1f) {
						isInPose = true;
					}
			...
			case Gestures.Wave:
				...
				case 0:  // gesture detection - phase 1
					...
					{
						SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex], 3, frameNumber);
					...
					{
						SetGestureJoint(ref gestureData, timestamp, leftHandIndex, jointsPos[leftHandIndex], 3, frameNumber);
						gestureData.progress = 0.3f;
					}
					break;
				...
				case 2:  // gesture phase 3 = complete
					if((timestamp - gestureData.timestamp) < 1.5f)
					{
						...
						{
							Vector3 jointPos = jointsPos[gestureData.joint];
							CheckPoseComplete(ref gestureData, timestamp, jointPos, isInPose, 0f, frameNumber);
						}
					}
					else
					{
			...
			case Gestures.Click:
				switch(gestureData.state) {
				case 0:  // gesture detection - phase 1
					if(jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] && (jointsPos[rightHandIndex].y - jointsPos[rightElbowIndex].y) > -0.1f) {
						SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex], 2, frameNumber);
						gestureData.progress = 0.3f;
					...
					//						else
				{
					...
					Vector3 jointPos = jointsPos[gestureData.joint];
					CheckPoseComplete(ref gestureData, timestamp, jointPos, isInPose, KinectWrapper.Constants.ClickStayDuration, frameNumber);
			...
			case Gestures.Jump:
				...
				case 0:  // gesture detection - phase 1
					...
					{
						SetGestureJoint(ref gestureData, timestamp, hipCenterIndex, jointsPos[hipCenterIndex], 2, frameNumber);
						gestureData.progress = 0.5f;
					}
					break;
			...
			case Gestures.Push:
				...
				case 0:  // gesture detection - phase 1
					if(jointsTracked[rightHandIndex] && jointsTracked[hipCenterIndex] &&
					   (jointsPos[rightHandIndex].y - jointsPos[hipCenterIndex].y) > -0.05f)/* &&
				   Mathf.Abs(jointsPos[rightHandIndex].x - jointsPos[hipCenterIndex].x) < 0.15f &&
				   (jointsPos[rightHandIndex].z - jointsPos[hipCenterIndex].z) < -0.05f)*/
					{
						SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex], 2, frameNumber);
						gestureData.progress = 0.5f;
					}
					else if(jointsTracked[leftHandIndex] && jointsTracked[hipCenterIndex] &&
					        (jointsPos[leftHandIndex].y - jointsPos[hipCenterIndex].y) > -0.05f)/* &&
				        Mathf.Abs(jointsPos[leftHandIndex].x - jointsPos[hipCenterIndex].x) < 0.15f &&
				        (jointsPos[leftHandIndex].z - jointsPos[hipCenterIndex].z) < -0.05f)*/
					{
						SetGestureJoint(ref gestureData, timestamp, leftHandIndex, jointsPos[leftHandIndex], 2, frameNumber);
						gestureData.progress = 0.5f;
					}
				...
				case 1:  // gesture phase 2 = complete
					if((timestamp - gestureData.timestamp) < 1.5f)
					{
						//Debug.Log ("Current: " + jointsPos[rightHandIndex].z.ToString() + " / Previous: " + gestureData.jointPos.z);
						bool isInPose = gestureData.joint == rightHandIndex ?
							jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] &&
