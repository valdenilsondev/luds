...
public class KinectModelControllerV2 : MonoBehaviourSingleton<KinectModelControllerV2> {
	...
	private List<SkeletonFrameEureka> frames = new List<SkeletonFrameEureka>();
	private SkeletonFrameEureka frameBase;
	...
	void Start () {
		...
		_bones = new GameObject[(int)KinectInte.NuiSkeletonPositionIndex.Count + 5] {
		...
		_baseRotation = new Quaternion[(int)KinectInte.NuiSkeletonPositionIndex.Count];
		_boneDir = new Vector3[(int)KinectInte.NuiSkeletonPositionIndex.Count];
		...
		for( int ii = 0; ii < (int)KinectInte.NuiSkeletonPositionIndex.Count; ii++) {
			if((_nullMask & (uint)(1 << ii)) <= 0) {
				...
				if(ii % 4 == 3 && ((_nullMask & (uint)(1 << (ii/4) + (int)KinectInte.NuiSkeletonPositionIndex.Count)) <= 0)) {
					_boneDir[ii] = _bones[(ii/4) + (int)KinectInte.NuiSkeletonPositionIndex.Count].transform.position - _bones[ii].transform.position;
				}
				//if the bone is the hip_override (at boneindex Hip_Left, get direction from average of left and right hips
				else if(ii == (int)KinectInte.NuiSkeletonPositionIndex.HipLeft && Hip_Left != null && Hip_Right != null) {
				...
				if(ii == (int)KinectInte.NuiSkeletonPositionIndex.Spine)
				{
		...
		_chestRight -= Vector3.Project(_chestRight, _boneDir[(int)KinectInte.NuiSkeletonPositionIndex.Spine]);
		//make _hipRight orthogonal to the direction of the hip override
		Vector3.OrthoNormalize(ref _boneDir[(int)KinectInte.NuiSkeletonPositionIndex.HipLeft],ref _hipRight);
	}
	...
	private void SaveSkeletonData() {
		frameBase = new SkeletonFrameEureka();

		for(int i = 0; i < _bones.Length; i++) {
			if(_bones[i] != null) {
				frameBase.SkeletonPositions[i] = _bones[i].transform.localPosition;
				frameBase.SkeletonRotations[i] = _bones[i].transform.localRotation;
			}
		}

		frames.Add(frameBase);
	}
	...
	public void SaveRecording() {
		animationCreation.SetAnimation(frames, _bones);
		frames = new List<SkeletonFrameEureka>();
	}
	...
	void RotateJoint(int bone) {
		...
		if(bone == (int)KinectInte.NuiSkeletonPositionIndex.Spine)
		{
		...
		if( sw.boneState[player,bone] == KinectInte.NuiSkeletonPositionTrackingState.NotTracked)
		{
			return;
		}
		...
		if(bone % 4 == 0)
		{
			...
			if(bone == (int)KinectInte.NuiSkeletonPositionIndex.HipLeft)
			{
				//target = vector from hip_center to average of hips left and right
				target = ((sw.bonePos[player,(int)KinectInte.NuiSkeletonPositionIndex.HipLeft] + sw.bonePos[player,(int)KinectInte.NuiSkeletonPositionIndex.HipRight]) / 2F) - sw.bonePos[player,(int)KinectInte.NuiSkeletonPositionIndex.HipCenter];
			}
			//otherwise it is one of the shoulders
			else
			{
				//target = vector from shoulder_center to bone
				target = sw.bonePos[player,bone] - sw.bonePos[player,(int)KinectInte.NuiSkeletonPositionIndex.ShoulderCenter];
			}
		}
		else
		{
		...
		if(bone == (int)KinectInte.NuiSkeletonPositionIndex.Spine)
		{
			...
			dir = _chestRight;
			target = sw.bonePos[player,(int)KinectInte.NuiSkeletonPositionIndex.ShoulderRight] - sw.bonePos[player,(int)KinectInte.NuiSkeletonPositionIndex.ShoulderLeft];
		...
		else if(bone == (int)KinectInte.NuiSkeletonPositionIndex.HipLeft)
		{
			//rotate the hips so they face forward (determined by the hips)
			dir = _hipRight;
			target = sw.bonePos[player,(int)KinectInte.NuiSkeletonPositionIndex.HipRight] - sw.bonePos[player,(int)KinectInte.NuiSkeletonPositionIndex.HipLeft];
		...
		if(bone == (int)KinectInte.NuiSkeletonPositionIndex.Spine) {
			restoreBone(_bones[(int)KinectInte.NuiSkeletonPositionIndex.HipLeft],_boneDir[(int)KinectInte.NuiSkeletonPositionIndex.HipLeft],upDir);
			restoreBone(_bones[(int)KinectInte.NuiSkeletonPositionIndex.HipLeft],_hipRight,rightDir);
		}
