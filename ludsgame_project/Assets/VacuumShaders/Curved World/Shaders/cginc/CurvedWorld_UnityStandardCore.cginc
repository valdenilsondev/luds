...
#include "../cginc/CurvedWorld_StandardLimits.cginc"


#ifdef V_CW_STANDARD_FORWARDADD_SM2
	#ifdef _TANGENT_TO_WORLD
	#undef _TANGENT_TO_WORLD
	#endif
#endif
...
#else
#define IN_LIGHTDIR_FWDADD(i) i.lightDir.xyz
#endif



#ifdef _TANGENT_TO_WORLD
#define FRAGMENT_SETUP(x,color) FragmentCommonData x = \
	FragmentSetup(color, i.tex, i.eyeVec, WorldNormal(i.tangentToWorldAndParallax), IN_VIEWDIR4PARALLAX(i), ExtractTangentToWorldPerPixel(i.tangentToWorldAndParallax), IN_WORLDPOS(i));
#else
#define FRAGMENT_SETUP(x,color) FragmentCommonData x = \
	FragmentSetup(color, i.tex, i.eyeVec, i.worldNormal.xyz, IN_VIEWDIR4PARALLAX(i), half3x3(0,0,0,0,0,0,0,0,0), IN_WORLDPOS(i));
#endif

#ifdef _TANGENT_TO_WORLD
#define FRAGMENT_SETUP_FWDADD(x,color) FragmentCommonData x = \
	FragmentSetup(color, i.tex, i.eyeVec, WorldNormal(i.tangentToWorldAndLightDir), IN_VIEWDIR4PARALLAX_FWDADD(i), ExtractTangentToWorldPerPixel(i.tangentToWorldAndLightDir), half3(0,0,0));
#else
#define FRAGMENT_SETUP_FWDADD(x,color) FragmentCommonData x = \
	FragmentSetup(color, i.tex, i.eyeVec, i.worldNormal.xyz, IN_VIEWDIR4PARALLAX_FWDADD(i), half3x3(0,0,0,0,0,0,0,0,0), half3(0,0,0));
#endif
...
{
	#ifdef V_CW_VERTEX_COLOR 
		FRAGMENT_SETUP(s,i.color)
	#else
		FRAGMENT_SETUP(s,fixed4(1, 1, 1, 1))
	#endif
	...
	c.rgb += Emission(i.tex.xy);
        ...
        //c.rgb += _V_CW_Rim_Color.rgb * pow (rim, _V_CW_Rim_Power);
		c.rgb = lerp(c.rgb, _V_CW_Rim_Color.rgb, pow (rim, _V_CW_Rim_Power));
	#endif
...
struct VertexOutputForwardAdd
{
	...
	#ifdef V_CW_VERTEX_COLOR
		#ifdef V_CW_STANDARD_FORWARDADD_SM2
			//d3d11_9x has no free register for COLOR in SM2
			//COLOR (rgb) is - fixed3(eyeVec.w, worldNormal.w, lightDir.w);
		#else
			fixed4 color						: COLOR0;
		#endif
	#endif
...
VertexOutputForwardAdd vertForwardAdd (VertexInput v)
{
	...
	float3 lightDir = _WorldSpaceLightPos0.xyz - posWorld.xyz * _WorldSpaceLightPos0.w;
		...
		o.viewDirForParallax = mul (rotation, ObjSpaceViewDir(v.vertex));
			...
			o.worldNormal.w = v.color.g;
			o.lightDir.w = v.color.b;
		#else
			o.color = v.color;
		#endif
	#endif
...
half4 fragForwardAdd (VertexOutputForwardAdd i) : SV_Target
{
	#ifdef V_CW_VERTEX_COLOR
		#ifdef V_CW_STANDARD_FORWARDADD_SM2
			FRAGMENT_SETUP_FWDADD(s,fixed4(i.eyeVec.w, i.worldNormal.w, i.lightDir.w, 1))
		#else
			FRAGMENT_SETUP_FWDADD(s,i.color)
		#endif
	#else
		FRAGMENT_SETUP_FWDADD(s,fixed4(1, 1, 1, 1))
	#endif
...
)
{
	...
	color += UNITY_BRDF_GI (s.diffColor, s.specColor, s.oneMinusReflectivity, s.oneMinusRoughness, s.normalWorld, -s.eyeVec, occlusion, gi);
		...
		half rim = 1.0 - saturate(dot (-s.eyeVec, s.normalWorld));
        //c.rgb += _V_CW_Rim_Color.rgb * pow (rim, _V_CW_Rim_Power);
		color.rgb = lerp(color.rgb, _V_CW_Rim_Color.rgb, pow (rim, _V_CW_Rim_Power));
	#endif
